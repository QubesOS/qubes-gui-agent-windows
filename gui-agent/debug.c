...
typedef struct _DBG_NAME_INFO
{
    char *name;
    DWORD id;
} DBG_NAME_INFO;

static DBG_NAME_INFO WindowsMessages[] =
{
    { "WM_NULL", 0x0000 },
    { "WM_CREATE", 0x0001 },
    { "WM_DESTROY", 0x0002 },
...
{
    int i;
    for (i = 0; i < RTL_NUMBER_OF(WindowsMessages); i++)
    {
        if (WindowsMessages[i].id == id)
            return WindowsMessages[i].name;
    }
    return "<UNKNOWN>";
}

static DBG_NAME_INFO WindowsHooks[] =
{
    { "WH_CALLWNDPROC", 4 },
    { "WH_GETMESSAGE", 3 },
    { "WH_CBT", 5 },
...
{
    int i;
    for (i = 0; i < RTL_NUMBER_OF(WindowsHooks); i++)
    {
        if (WindowsHooks[i].id == id)
            return WindowsHooks[i].name;
    }
    return "<UNKNOWN>";
}

static DBG_NAME_INFO CBTCodes[] =
{
    { "HCBT_MOVESIZE", 0 },
    { "HCBT_MINMAX", 1 },
    { "HCBT_QS", 2 },
...
{
    int i;
    for (i = 0; i < RTL_NUMBER_OF(CBTCodes); i++)
    {
        if (CBTCodes[i].id == id)
            return CBTCodes[i].name;
    }
    return "<UNKNOWN>";
}

#define STYLE_CHECK(ws) if (style & ws) { LogDebugRaw(#ws); LogDebugRaw(" "); }
